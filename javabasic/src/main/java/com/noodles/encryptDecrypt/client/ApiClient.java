package com.noodles.encryptDecrypt.client;
...
import com.noodles.encryptDecrypt.util.AESUtil;
import com.noodles.encryptDecrypt.util.Base64;
import com.noodles.encryptDecrypt.util.RSAUtil;
import com.noodles.encryptDecrypt.util.ZipUtil;
...
public class ApiClient {
	...
	// RSA公钥
	private java.security.PublicKey rsa_public_key = null;

	// RSA私钥
	private java.security.PrivateKey rsa_private_key = null;
	...
	public FileUploadResponse execute(FileUploadRequest req) {
		...
		try {
			// 压缩后的zip数据文件
			zipFile = new File(req.getTargetFilePath() + File.separator
					+ srcFile.getName().substring(0, srcFile.getName().lastIndexOf(".")) + ".zip");
			// 压缩文件
