...
/**
 * @author +,-,*,/四则运算的表达式逆波兰解析计算类,精确计算,应用BigDecimal类处理
...
public class ExpArithmetic {
	...
	public ExpArithmetic() {
	}

	public ExpArithmetic(String expBase) {
		init(expBase, this.precision, this.roundingMode);
	}

	public ExpArithmetic(String expBase, int precision,
	...
	public static boolean compareTo(String strParm) {
		...
		if (-1 != str.indexOf(">=")) {
			...
			int r = new BigDecimal((new ExpArithmetic(strTemps[0]).caculate()))
					.compareTo(new BigDecimal((new ExpArithmetic(strTemps[1])
							.caculate())));
			if (-1 == r) {
				reBoolean = false;
			} else {
	...
	public static void main(String... args) {
		//
		ExpArithmetic me = new ExpArithmetic("275.0+300.0");
		...
		// // System.out.println("------------------------------------");
		// // Pattern p =
		// Pattern.compile("^\\([\\s\\S]*\\)$");//匹配类似以'('开头')'结尾的字符串
