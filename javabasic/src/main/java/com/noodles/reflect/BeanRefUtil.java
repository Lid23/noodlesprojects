...
public class BeanRefUtil {
	...
	/**
	 * @param bean
	 * @param encryptFieldList
	 * @return
	 * 作者：KJ00019
	 * 日期：2017年9月12日下午3:14:54
	 */
	public static Map<String, Object> getFieldValueMap(Object bean, List<String> encryptFieldList) {
		...
		for (Field field : fields) {
			try {
				...
				if (!checkMet(methods, fieldGetName)) {
					continue;
				}
...
//				String fieldKeyName = parKeyName(field.getName());
	 ...
	 * 对需要解密的字段进行解密
	 * @param bean
	 * @param decryotFieldList
	 * @return
	 * 作者：KJ00019
	 * 日期：2017年9月12日上午11:12:02
	 */
	public static Object decryptBeanField(Object bean, List<String> decryotFieldList) {
		Class<?> cls = bean.getClass();
		Method[] methods = cls.getDeclaredMethods();
		Field[] fields = cls.getDeclaredFields();
		for (Field field : fields) {
			try {
				...
				if (!checkMet(methods, filedSetName)) {
					continue;
				}
				Method fieldGetMet = cls.getMethod(fieldGetName, new Class[] {});
				Method fieldSetMet = cls.getMethod(filedSetName, field.getType());
				Object fieldVal = fieldGetMet.invoke(bean, new Object[] {});
				
				if(decryotFieldList.contains(fieldName) && "String".equals(fieldType)){
					fieldSetMet.invoke(bean, decryptString(fieldVal.toString()));
				}
